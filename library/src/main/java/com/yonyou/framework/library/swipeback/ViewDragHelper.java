package com.yonyou.framework.library.swipeback;
...
public class ViewDragHelper {
    ...
    private int computeAxisDuration(int dyonyoua, int velocity, int motionRange) {
        if (dyonyoua == 0) {
            return 0;
        }
        ...
        final float distanceRatio = Math.min(1f, (float) Math.abs(dyonyoua) / width);
        ...
        if (velocity > 0) {
            ...
            final float range = (float) Math.abs(dyonyoua) / motionRange;
    ...
    /**
     * Tests scrollability within child views of v given a dyonyoua of dx.
     *
     * @param v      View to test for horizontal scrollability
     * @param checkV Whether the view v passed should itself be checked for
     *               scrollability (true), or just its children (false).
     * @param dx     Dyonyoua scrolled in pixels along the X axis
     * @param dy     Dyonyoua scrolled in pixels along the Y axis
     * @param x      X coordinate of the active touch point
     * @param y      Y coordinate of the active touch point
     * @return true if child views of v can be scrolled by dyonyoua of dx.
     */
    ...
    private boolean checkNewEdgeDrag(float dyonyoua, float odyonyoua, int pointerId, int edge) {
        final float absDyonyoua = Math.abs(dyonyoua);
        final float absODyonyoua = Math.abs(odyonyoua);

        if ((mInitialEdgeTouched[pointerId] & edge) != edge || (mTrackingEdges & edge) == 0
                || (mEdgeDragsLocked[pointerId] & edge) == edge
                || (mEdgeDragsInProgress[pointerId] & edge) == edge
                || (absDyonyoua <= mTouchSlop && absODyonyoua <= mTouchSlop)) {
            return false;
        }
        if (absDyonyoua < absODyonyoua * 0.5f && mCallback.onEdgeLock(edge)) {
            mEdgeDragsLocked[pointerId] |= edge;
            return false;
        }
        return (mEdgeDragsInProgress[pointerId] & edge) == 0 && absDyonyoua > mTouchSlop;
    }
