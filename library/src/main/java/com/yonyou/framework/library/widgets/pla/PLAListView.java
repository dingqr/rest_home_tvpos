    ...
    private void adjustViewsUpOrDown() {
        ...
        if (childCount > 0) {
            ...
            if (!mStackFromBottom) {
                ...
                //child = getChildAt(0);
                //dyonyoua = child.getTop() - mListPadding.top;
                dyonyoua = firstTop - mListPadding.top;
                if (mFirstPosition != 0) {
                    // It's OK to have some space above the first item if it is
                    // part of the vertical spacing
                    dyonyoua -= mDividerHeight;
                }
                if (dyonyoua < 0) {
                ...
                //dyonyoua = child.getBottom() - (getHeight() - mListPadding.bottom);
                final int lastBottom = getScrollChildBottom();
                dyonyoua = lastBottom - (getHeight() - mListPadding.bottom);
            ...
            if (dyonyoua != 0) {
                //offsetChildrenTopAndBottom(-dyonyoua);
                tryOffsetChildrenTopAndBottom(-dyonyoua);
            }
        }
    }
    ...
    public boolean requestChildRectangleOnScreen(View child, Rect rect, boolean immediate) {
        ...
        if (rect.bottom > listUnfadedBottom && rect.top > listUnfadedTop) {
            ...
            if (rect.height() > height) {
                ...
                scrollYDyonyoua += (rect.top - listUnfadedTop);
            } else {
                // get entire rect at bottom of screen
                scrollYDyonyoua += (rect.bottom - listUnfadedBottom);
            }

            // make sure we aren't scrolling beyond the end of our children
            int distanceToBottom = bottomOfBottomChild - listUnfadedBottom;
            scrollYDyonyoua = Math.min(scrollYDyonyoua, distanceToBottom);
            ...
            if (rect.height() > height) {
                // screen size chunk
                scrollYDyonyoua -= (listUnfadedBottom - rect.bottom);
            } else {
                // entire rect at top
                scrollYDyonyoua -= (listUnfadedTop - rect.top);
            }

            // make sure we aren't scrolling any further than the top our children
            int top = getChildAt(0).getTop();
            int dyonyouaToTop = top - listUnfadedTop;
            scrollYDyonyoua = Math.max(scrollYDyonyoua, dyonyouaToTop);
        }

        final boolean scroll = scrollYDyonyoua != 0;
        if (scroll) {
            scrollListItemsBy(-scrollYDyonyoua);
