        ...
        viewHolder.reduce.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (num > 0) {
                    ...
                    if(num==0){
                        shopProducts.get(position).setNumber(num);
                        shopToDetailListener.onRemoveProduct(shopProducts.get(position));
                    }else {
                        shopProducts.get(position).setNumber(num);
                        viewHolder.shoppingNum.setText(shopProducts.get(position).getNumber()+"");
                        if (shopToDetailListener != null) {
...
//        public TextView commodityNum;
    ...
    private boolean judgeNumber(int number){
        ...
        for (int i = 0; i < Integer.MAX_VALUE; i++){
            if (returnNumber(number) != 0){
                number = number/returnNumber(number);
                if (excuteNumber(number)){
                    return true;
                }
            }else{
                return false;
    ...
    private int returnNumber(int numb){
        if (numb%2 == 0){
            return  2;
        }
    ...
    private boolean excuteNumber(int num){
        if (num/2 == 1){
            return true;
        }
