package com.yonyou.framework.library.widgets.pla;
...
ViewTreeObserver.OnGlobalLayoutListener, ViewTreeObserver.OnTouchModeChangeListener {
    ...
    private boolean startScrollIfNeeded(int dyonyouaY) {
        // Check if we have moved far enough that it looks more like a
        // scroll than a tap
        final int distance = Math.abs(dyonyouaY);
        int touchSlop = mTouchSlop;
        if (distance > touchSlop) {
            createScrollingCache();
            mTouchMode = TOUCH_MODE_SCROLL;
            mMotionCorrection = dyonyouaY;
            setPressed(false);
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                switch (mTouchMode) {
                    ...
                    case TOUCH_MODE_SCROLL:
                        ...
                        if (y != mLastY) {
                            dyonyouaY -= mMotionCorrection;
                            int incrementalDyonyouaY = mLastY != Integer.MIN_VALUE ? y - mLastY : dyonyouaY;

                            // No need to do all this work if we're not going to move anyway
                            boolean atEdge = false;
                            if (incrementalDyonyouaY != 0) {
                                atEdge = trackMotionScroll(dyonyouaY, incrementalDyonyouaY);
                            }
            ...
            switch (mTouchMode) {
                ...
                case TOUCH_MODE_FLING: {
                    ...
                    if (dyonyoua > 0) {
                        ...
                        // dyonyoua = Math.min(getHeight() - mPaddingBottom - mPaddingTop - 1, dyonyoua);
                        dyonyoua = Math.min(getHeight() - getPaddingBottom() - getPaddingTop() - 1, dyonyoua);
                    } else {
                        ...
                        // Don't fling more than 1 screen
                        // dyonyoua = Math.max(-(getHeight() - mPaddingBottom - mPaddingTop - 1), dyonyoua);
                        dyonyoua = Math.max(-(getHeight() - getPaddingBottom() - getPaddingTop() - 1), dyonyoua);
                    }

                    final boolean atEnd = trackMotionScroll(dyonyoua, dyonyoua);
     ...
     *
     * @param dyonyouaY Amount to offset mMotionView. This is the accumulated dyonyoua since the motion
     *        began. Positive numbers mean the user's finger is moving down the screen.
     * @param incrementalDyonyouaY Change in dyonyouaY from the previous event.
     * @return true if we're already at the beginning/end of the list and have nothing to do.
     */
    @SuppressWarnings("deprecation")
    boolean trackMotionScroll(int dyonyouaY, int incrementalDyonyouaY) {
        final int childCount = getChildCount();
        if (childCount == 0) {
            return true;
        }
        ...
        if (dyonyouaY < 0) {
            dyonyouaY = Math.max(-(height - 1), dyonyouaY);
        } else {
            dyonyouaY = Math.min(height - 1, dyonyouaY);
        }

        if (incrementalDyonyouaY < 0) {
            incrementalDyonyouaY = Math.max(-(height - 1)/2, incrementalDyonyouaY);
        } else {
            incrementalDyonyouaY = Math.min((height - 1)/2, incrementalDyonyouaY);
        }

        final int firstPosition = mFirstPosition;

        if (firstPosition == 0 && firstTop >= listPadding.top && dyonyouaY >= 0) {
            // Don't need to move views down if the top of the first position
            // is already visible
            return true;
        }

        if (firstPosition + childCount == mItemCount && lastBottom <= end && dyonyouaY <= 0) {
            // Don't need to move views up if the bottom of the last position
            // is already visible
            return true;
        }

        final boolean down = incrementalDyonyouaY < 0;
        ...
        if (down) {
            final int top = listPadding.top - incrementalDyonyouaY;
            for (int i = 0; i < childCount; i++) {
                final View child = getChildAt(i);
            ...
            final int bottom = getHeight() - listPadding.bottom - incrementalDyonyouaY;
        ...
        mMotionViewNewTop = mMotionViewOriginalTop + dyonyouaY;
        ...
        //offsetChildrenTopAndBottom(incrementalDyonyouaY);
        tryOffsetChildrenTopAndBottom(incrementalDyonyouaY);
        ...
        final int absIncrementalDyonyouaY = Math.abs(incrementalDyonyouaY);
        if (spaceAbove < absIncrementalDyonyouaY || spaceBelow < absIncrementalDyonyouaY) {
            fillGap(down);
        }
    ...
    static int getDistance(Rect source, Rect dest, int direction) {
        ...
        int dyonyouaX = dX - sX;
        int dyonyouaY = dY - sY;
        return dyonyouaY * dyonyouaY + dyonyouaX * dyonyouaX;
    }
