...
public class MonthDateView extends View {
    ...
    private int mLineColor = Color.parseColor("#c5c5c5");
    ...
    public MonthDateView(Context context, AttributeSet attrs) {
        ...
        mWeekDayColor = typedArray.getColor(R.styleable.CalendarView_WeekDayColor, Color.parseColor("#f2f1f1"));
        ...
        mCurrentSize = (int) typedArray.getDimension(R.styleable.CalendarView_CurrentSize, 12);
        typedArray.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int column, row = 0;
        //绘制线条
        drawLines(canvas, (weekNumber - 1) % 7);
        for (int day = 0; day < mMonthDays; day++) {
            ...
            int maxRowIndex = getMonthRowNumber() - 1 ;
            float startRecX = mColumnSize * maxColumnIndex + mMarginSize;
            float startRecY = mRowSize * maxRowIndex + mMarginSize;
            float endRecX = startRecX + mColumnSize - 2 * mMarginSize;
            float endRecY = startRecY + mRowSize - 2 * mMarginSize;
            mPaint.setColor(mWeekDayColor);
            mPaint.setStyle(Style.FILL);
            ...
            if (dayString.equals(mSelDay + "")) {
                ...
                canvas.drawText("今", mColumnSize * column + 12, mRowSize * row + 33, mPaint);
            } else {
                String price = isdayAndPriceList(day + 1);
                if (!TextUtils.isEmpty(price)) {
                    int dateY = (int) (startY);
     ...
     * @param canvas
     */
    private void drawLines(Canvas canvas, int startColumn) {
        int rightX = getWidth();
        ...
        for (int row = 0; row <= rowCount; row++) {
            ...
            // 控制一条横线的宽度
            if (row == 0){
                startX = startColumn * mColumnSize;
            }else {
                startX = 0;
            }
