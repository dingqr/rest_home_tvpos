...
//        Elog.e("canteenTableEntity=" + canteenTableEntity.tableName + "Pos=" + mActualPos + "tableStatus=" + canteenTableEntity.tableStatus);
...
//        //操作时间
        ...
        //账单金额
        if (!TextUtils.isEmpty(canteenTableEntity.getBillMoney())) {
            holder.setText(R.id.tv_money, mContext.getResources().getString(R.string.RMB_symbol) + canteenTableEntity.getBillMoney());
        } else {
            holder.setText(R.id.tv_money, mContext.getResources().getString(R.string.RMB_symbol) + "0.00");
        }
        ...
        //并台标记-默认隐藏
        holder.setVisible(R.id.tv_muti_sign, false);
        //除了空闲状态，都显示操作时间
        holder.setVisible(R.id.tv_make_time, true);
        //设置选中效果
        ...
        handleTableStatus(holder, canteenTableEntity);
    }

    /**
     * @param holder             根据桌台状态显示桌台
     * @param canteenTableEntity 桌台状态：0 空闲 1 占用 2 清扫 3 预定 4 预结账 5 被并台 6 占用（消费中）
     *                           占用（部分支付）8 占用（锁定）9 占用（结清） 10占用（未消费），
     *                           传（6,7,8,9,10）为查询占用，不传默认查询全部
     */
    private void handleTableStatus(ViewHolder holder, CanteenTableEntity canteenTableEntity) {
        switch (canteenTableEntity.tableStatus) {
            //空闲
            case 0:
                holder.setVisible(R.id.iv_locked, false);
            ...
            case 5:
                holder.setVisible(R.id.iv_locked, false);
                holder.setVisible(R.id.tv_muti_sign, true);
                holder.setVisible(R.id.iv_locked, true);
                holder.setVisible(R.id.tv_person_num, true);
                holder.setVisible(R.id.tv_seat_num, true);
                break;
            //占用-未结清
            case 6:
            case 7:
            ...
            case 10:
                ...
                //锁定小icon
                holder.setVisible(R.id.iv_locked, true);
                holder.setImageResource(R.id.iv_locked, R.drawable.ic_table_locked);
            ...
            case 9:
                ...
                holder.setVisible(R.id.tv_seat_num, false);
                holder.setVisible(R.id.iv_locked, true);
                holder.setImageResource(R.id.iv_locked, R.drawable.ic_table_settled);
