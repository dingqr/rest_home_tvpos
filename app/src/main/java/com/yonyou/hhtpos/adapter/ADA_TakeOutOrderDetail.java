 ...
 * 描述：菜品明细列表-展示的实体类
 */
public class ADA_TakeOutOrderDetail extends CommonAdapterListView<WMDishDetailListEntity> {
    ...
    @Override
    protected void convert(ViewHolderListView holder, WMDishDetailListEntity dishDetailEntity, int position) {
        ...
        if (dishDetailEntity != null) {
            ...
            holder.setText(R.id.tv_dish_price, mContext.getResources().getString(R.string.RMB_symbol) + dishDetailEntity.getDishPrice());
    ...
    private int getFirstVisiblePosition(String orderTime) {
        for (int i = 0; i < mDatas.size(); i++) {
            WMDishDetailListEntity wmDishDetailEntity = mDatas.get(i);
